...
const MetricCard: React.FC<MetricCardProps> = ({ 
  ...
  return (
    <div className={`${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white'} rounded-xl shadow-lg p-6 transition-all duration-200 hover:shadow-xl border-l-4 ${color}`}>
...
const Dashboard: React.FC = () => {
  ...
  const PTC_COLORS = ['#ffcccb', '#ffd700', '#90ee90', '#00bfff']; // Red to Blue gradient for PTC metrics
  ...
  if (error) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <p className="text-xl text-red-500">Error loading data: {error.message}</p>
      </div>
    );
  }
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex space-x-2">
                <button className="px-4 py-2 rounded-lg bg-white dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200">
                  Export
                </button>
                <button className="px-4 py-2 rounded-lg bg-blue-500 hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700 text-white transition-colors duration-200">
                  Refresh Data
                </button>
              </div>
            </div>
              ...
              <ChartContainer title="Bottom 10 Locomotives by PTC Active %">
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={data.bottomByPtcPercentage} layout="vertical">
                    ...
                    <Bar dataKey="value" fill="#FF8042" name="PTC Active %" />
                  </BarChart>
                </ResponsiveContainer>
              </ChartContainer>
            </div>
